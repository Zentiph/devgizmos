...
class Char:
    ...
    def __add__(self, value, /):
        if not isinstance(value, (str, Char)):
            raise TypeError(
                f"Unsupported operand types for + ('Char' and '{type(value).__name__}')"
            )
...
class Number:
    ...
    Number
    ======
    Represents an integer or float number, as well as other related values like infinity.
    """
    ...
    def __init__(self, value: Any):
        ...
        :param value: The value to convert to a number. Must have an __int__ or __float__ method
        or be a specific str such as "inf" or "infinity".
        :type value: Any
        """

        if not hasattr(value, "__int__") and not hasattr(value, "__float__"):
            raise TypeError(
                "Value to convert to a Num must have an __int__ or __float__ method"
            )

        val: Union[int, float]

        # if value can be converted to a float but not an int
        if hasattr(value, "__float__"):
            # if the float is an integer convert it to an int
